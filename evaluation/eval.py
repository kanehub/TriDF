...
def eval_train(
    ...
    for i, test_data in enumerate(test_loader):
        ...
        if save_image:
            ...
            # acc_pred_im = acc_pred.detach().cpu()
            # acc_pred_colored = (colorize_np(acc_pred_im, range=(0., 1.)) * 255).astype(np.uint8)
            # imageio.imwrite(os.path.join(pred_img_dir, 'acc_{:03d}.png'.format(i)), acc_pred_colored)
        ...
        # imageio.imwrite(os.path.join(pred_img_dir, '{}_acc_map_coarse.png'.format(file_id)),
    ...
    with open("{}/result_{}_{}.json".format(test_out_dir, args.scene_id, iteration), 'w') as f:
        ...
        # write_dict_to_txt(results_dict,file=f)
